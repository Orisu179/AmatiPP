...
{
    if ((fMidiIn = juce::MidiInput::openDevice (juce::MidiInput::getDefaultDevice().identifier, this)) == nullptr)
    {
        return false;
    }
    if ((fMidiOut = juce::MidiOutput::openDevice (juce::MidiOutput::getDefaultDevice().identifier)) == nullptr)
    {
        return false;
    }
    fMidiIn->start();
